...
public class CommentV2Controller {
  ...
  @ApiOperation(value = "{hashcode}에 해당하는 초대장의 댓글들을 불러오는 API")
  @GetMapping("/apis/v2/invitations/{hashcode}/comments")
  public ResponseEntity<GetCommentResponseDto> getComment(
      @PathVariable(value = "hashcode") String hashcode
  ) {
   return ResponseEntity.status(HttpStatus.OK).body(commentV2Service.getComment(hashcode));
  }
